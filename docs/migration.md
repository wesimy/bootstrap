...
This list highlights key changes by component between v3.x.x and v4.0.0-alpha.8.1.
...
All `@screen-` variables have been removed in v4.0.0-alpha.8.1. Use the `media-breakpoint-up()`, `media-breakpoint-down()`, or `media-breakpoint-only()` Sass mixins or the `$grid-breakpoints` Sass map instead.
